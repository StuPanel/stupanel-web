              ...
              <div className="flex rounded-lg border border-slate-200 overflow-hidden text-xs font-semibold">
                <button onClick={() => setLogoMode("upload")}
                  className={cn("flex-1 py-2 flex items-center justify-center gap-1.5 transition-colors",
                    logoMode === "upload" ? "bg-indigo-600 text-white" : "text-slate-500 hover:bg-slate-50")}>
                  <Upload className="w-3.5 h-3.5" /> Upload
                </button>
                <button onClick={() => setLogoMode("url")}
                  className={cn("flex-1 py-2 flex items-center justify-center gap-1.5 transition-colors",
                    logoMode === "url" ? "bg-indigo-600 text-white" : "text-slate-500 hover:bg-slate-50")}>
  ...
  return (
    <div className="max-w-4xl mx-auto">
        ...
        {tabs.map(t => {
          ...
          return (
            <button key={t.id} onClick={() => setTab(t.id)}
              className={cn(
                "flex items-center gap-2 px-3.5 py-2 rounded-xl border text-sm font-medium transition-all flex-shrink-0",
                active
      ...
      <div className="flex gap-6 items-start">
        ...
        <nav className="hidden md:flex flex-col w-52 flex-shrink-0 bg-white border border-slate-200 rounded-2xl p-2 shadow-sm sticky top-20">
          {tabs.map(t => {
            const Icon = t.icon;
            const active = tab === t.id;
            return (
